...
class CottonMoveItTrajectories(object):
  ...
  def __init__(self):
    ...
    try:
      ...
      self.cotton_orientation_vector = None
      rospy.Subscriber('/cotton_orientation_vector', Vector3Stamped, self.orientation_callback)
  ...
  def orientation_callback(self, msg):
    self.cotton_orientation_vector = msg
  ...
  def reach_joint_angles(self, tolerance=0.001):

    go_to_home_position(arm_group)
...
def handle_recheck(request):
  ...
  x = manipulation_re_check.cotton_orientation_vector.vector.x
  y = manipulation_re_check.cotton_orientation_vector.vector.y

  angle_rad = atan2(x, y)  #Swapped to make 0° = up
  ...
  if -22.5 <= angle_deg <= 22.5:
      ...
      aux_frame_number = None  # not happen
        
  if aux_frame_number is not None:
      target_frame = f"aux_frame_{aux_frame_number}"
      rospy.loginfo(f"Using target frame: {target_frame}")
